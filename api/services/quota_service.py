"""Quota checking service for Dograh credits + TAMALE billing.

Merged file: upstream Dograh quota logic (preserves all required exports for
v1.35.0 callers) plus TAMALE plan/credit gate that runs on every workflow start.

TAMALE check is fail-open: if TAMALE's voice_quota endpoint is unreachable or
errors, the call proceeds. This protects voice availability when TAMALE is
temporarily down.
"""
...
def _tamale_quota_denied_result(tamale_result: dict) -> "QuotaCheckResult":
...
async def authorize_workflow_run_start(
    *,
    ...
    TAMALE addition: a plan/voice-minutes gate runs against TAMALE's
    /api/v1/voice_quota/check endpoint before the upstream Dograh checks.
    Fail-open on any error so TAMALE outages don't break voice.
    """
    try:
        ...
        # ── TAMALE plan/voice-minutes gate ──
        if workflow.organization_id:
            tamale_result = await _check_tamale_quota(workflow.organization_id)
            if not tamale_result.get("allowed", True):
                return _tamale_quota_denied_result(tamale_result)
...
# ──────────────────────────────────────────────────────────────────────
# Backward-compat shims (kept for any non-upstream callers that still
# reference these older entry points)
# ──────────────────────────────────────────────────────────────────────
async def check_dograh_quota(
    user: UserModel, workflow_id: int | None = None
) -> QuotaCheckResult:
    """Compat shim: delegates to authorize_workflow_run_start."""
    if workflow_id is None:
        # No workflow context => we can still gate on TAMALE per user's org
    ...
    return await authorize_workflow_run_start(
        workflow_id=workflow_id, actor_user=user
    )
...
async def check_dograh_quota_by_user_id(
    ...
    """Compat shim: delegates to check_dograh_quota."""
